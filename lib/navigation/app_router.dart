...
GoRouter buildRouter(AuthProvider auth) => GoRouter(
      ...
      initialLocation: '/',
      refreshListenable: auth, // ← ini yang bikin redirect jalan saat logout
      redirect: (context, state) {
        // Jangan redirect saat status masih unknown (loading awal)
        ...
        // Sudah login → buka login/register → redirect home
      ...
      routes: [
        ...
        GoRoute(
          ...
          builder: (c, s) => DonationSuccessScreen(
            donationId: int.tryParse(s.uri.queryParameters['donation_id'] ?? ''),
          ),
        ),
        GoRoute(
          path: '/admin',
